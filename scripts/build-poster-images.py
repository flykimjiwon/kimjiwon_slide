...
def page1():
    ...
    shadow_box(img,(58,654,504,860),24,(239,246,255,245),(191,219,254,160),shadow=(0,0,0,40)); d.text((86,680),'웹 챗봇이 아니라',font=font(30),fill=TEXT); d.text((86,720),'코드베이스 안에서',font=font(30),fill=BLUE); d.text((86,760),'실제로 작업하는 도구',font=font(30),fill=BLUE); draw_text(d,(86,810),'프로젝트를 읽고 파일·Shell·git 도구를 호출하며, 수정과 검증 루프까지 이어갑니다.',font(18),fill=MUTED,max_w=372)
...
def page2():
    ...
    y=870
    for name,val,col,display in bars:
        ...
        rounded(d,(230,y+3,430,y+25),11,(234,240,248,255),None)
...
def page3():
    ...
    for t,v,s in metrics:
        shadow_box(img,(x,974,x+222,1138),20,WHITE,LINE,shadow=(15,23,42,16))
        d.text((x+18,998),t,font=font(16),fill=MUTED)
        d.text((x+18,1034),v,font=font(36),fill=BLUE2)
        if ' / ' in s:
            left, right = s.split(' / ', 1)
            small_f = font(11)
            d.text((x+18,1092),left,font=small_f,fill=BLUE)
            lx = x+18 + d.textlength(left, font=small_f)
            d.text((lx+4,1092),'/ ',font=small_f,fill=MUTED)
            rx = lx + 4 + d.textlength('/ ', font=small_f)
            d.text((rx,1092),right,font=small_f,fill=GREEN)
        else:
            d.text((x+18,1092),s,font=font(11),fill=MUTED)
        x+=246
